...
CombatManager::CombatManager ()
{
  ...
  for (int i = 0; i < AMNT_ARR_SHORT; i++)
  {
		arr_short_list1_.push_front(new Arrow(Left,
		CENTER_SPAWN_NE_X - OFFSET_ARR_SHORT_W, CENTER_SPAWN_NE_Y + OFFSET_ARR_SHORT_H));
		
		arr_short_list2_.push_front(new Arrow(Right,
		CENTER_SPAWN_NE_X + OFFSET_ARR_SHORT_W, CENTER_SPAWN_NE_Y + OFFSET_ARR_SHORT_H));
		
		arr_short_list3_.push_front(new Arrow(Left,
		CENTER_SPAWN_NW_X - OFFSET_ARR_SHORT_W, CENTER_SPAWN_NW_Y + OFFSET_ARR_SHORT_H));
		
		arr_short_list4_.push_front(new Arrow(Right,
		CENTER_SPAWN_NW_X + OFFSET_ARR_SHORT_W, CENTER_SPAWN_NW_Y + OFFSET_ARR_SHORT_H));
		
		arr_short_list5_.push_front(new Arrow(Left,
		CENTER_SPAWN_SE_X - OFFSET_ARR_SHORT_W, CENTER_SPAWN_SE_Y + OFFSET_ARR_SHORT_H));
		
		arr_short_list6_.push_front(new Arrow(Right,
		CENTER_SPAWN_SE_X + OFFSET_ARR_SHORT_W, CENTER_SPAWN_SE_Y + OFFSET_ARR_SHORT_H));
		
		arr_short_list7_.push_front(new Arrow(Left,
		CENTER_SPAWN_SW_X - OFFSET_ARR_SHORT_W, CENTER_SPAWN_SW_Y + OFFSET_ARR_SHORT_H));
		
		arr_short_list8_.push_front(new Arrow(Right,
		CENTER_SPAWN_SW_X + OFFSET_ARR_SHORT_W, CENTER_SPAWN_SW_Y + OFFSET_ARR_SHORT_H));
	}
	
	for (int j = 0; j < AMNT_ARR_LONG; j++)
  {
  	arr_long_list1_.push_front(new Arrow(Up,
		CENTER_SPAWN_MID_X + OFFSET_ARR_LONG_W, CENTER_SPAWN_MID_Y - OFFSET_ARR_LONG_H));
		
		arr_long_list2_.push_front(new Arrow(Down,
		CENTER_SPAWN_MID_X + OFFSET_ARR_LONG_W, CENTER_SPAWN_MID_Y + OFFSET_ARR_LONG_H));
	}
	
	arr_general_list_.push_front(arr_short_list1_);
	arr_general_list_.push_front(arr_short_list2_);
	arr_general_list_.push_front(arr_short_list3_);
	arr_general_list_.push_front(arr_short_list4_);
	arr_general_list_.push_front(arr_short_list5_);
	arr_general_list_.push_front(arr_short_list6_);
	arr_general_list_.push_front(arr_short_list7_);
	arr_general_list_.push_front(arr_short_list8_);
	arr_general_list_.push_front(arr_long_list1_);
	arr_general_list_.push_front(arr_long_list2_);
...
CombatManager::~CombatManager()
{
	...
	for(auto list_arr : arr_general_list_)
	{
		for(auto arr : list_arr)
		{
			delete arr;
		}
	}
...
void CombatManager::manageMovAllArrList()
{
	manageMovArrList(arr_short_list1_,
	CENTER_SPAWN_NE_X - OFFSET_ARR_SHORT_W, CENTER_SPAWN_NE_Y + OFFSET_ARR_SHORT_H,
	DIST_LIM_SHORT,  LAUNCH_DELAY_SHORT);
	
	manageMovArrList(arr_short_list2_,
	CENTER_SPAWN_NE_X + OFFSET_ARR_SHORT_W, CENTER_SPAWN_NE_Y + OFFSET_ARR_SHORT_H,
	DIST_LIM_SHORT,  LAUNCH_DELAY_SHORT);
	
	manageMovArrList(arr_short_list3_,
	CENTER_SPAWN_NW_X - OFFSET_ARR_SHORT_W, CENTER_SPAWN_NW_Y + OFFSET_ARR_SHORT_H,
	DIST_LIM_SHORT,  LAUNCH_DELAY_SHORT);
	
	manageMovArrList(arr_short_list4_,
	CENTER_SPAWN_NW_X + OFFSET_ARR_SHORT_W, CENTER_SPAWN_NW_Y + OFFSET_ARR_SHORT_H,
	DIST_LIM_SHORT,  LAUNCH_DELAY_SHORT);
	
	manageMovArrList(arr_short_list5_,
	CENTER_SPAWN_SE_X - OFFSET_ARR_SHORT_W, CENTER_SPAWN_SE_Y + OFFSET_ARR_SHORT_H,
	DIST_LIM_SHORT,  LAUNCH_DELAY_SHORT);
	
	manageMovArrList(arr_short_list6_,
	CENTER_SPAWN_SE_X + OFFSET_ARR_SHORT_W, CENTER_SPAWN_SE_Y + OFFSET_ARR_SHORT_H,
	DIST_LIM_SHORT,  LAUNCH_DELAY_SHORT);
	
	manageMovArrList(arr_short_list7_,
	CENTER_SPAWN_SW_X - OFFSET_ARR_SHORT_W, CENTER_SPAWN_SW_Y + OFFSET_ARR_SHORT_H,
	DIST_LIM_SHORT,  LAUNCH_DELAY_SHORT);
	
	manageMovArrList(arr_short_list8_,
	CENTER_SPAWN_SW_X + OFFSET_ARR_SHORT_W, CENTER_SPAWN_SW_Y + OFFSET_ARR_SHORT_H,
	DIST_LIM_SHORT,  LAUNCH_DELAY_SHORT);
	
	manageMovArrList(arr_long_list1_,
	CENTER_SPAWN_MID_X + OFFSET_ARR_LONG_W, CENTER_SPAWN_MID_Y - OFFSET_ARR_LONG_H,
	DIST_LIM_LONG,  LAUNCH_DELAY_LONG);
	
	manageMovArrList(arr_long_list2_,
	CENTER_SPAWN_MID_X + OFFSET_ARR_LONG_W, CENTER_SPAWN_MID_Y + OFFSET_ARR_LONG_H,
	DIST_LIM_LONG,  LAUNCH_DELAY_LONG);
}
...
void CombatManager::displayAllArrowList(sf::RenderTarget &rt)
{
	for(auto arr_list : arr_general_list_)
	{
		displayArrowList(rt, arr_list);
	}
}
...
void CombatManager::checkDamageArrow(std::list<Arrow *> arr_list, PlayableChar *pc, int damage)
{
	for(auto arr : arr_list)
	{
		if(isInRadius2D(pc, arr->getX(), arr->getY(), 5))
		{
			pc->setHealth(pc->getHealth() - damage);
		}
	}
...
void CombatManager::checkDamageAllArrowList(PlayableChar *pc, int damage)
{
	for(auto arr_list : arr_general_list_)
	{
		checkDamageArrow(arr_list, pc, damage);
	}
}
...
void CombatManager::checkDamages(PlayableChar * pc)
{
	...
	checkDamageAllArrowList(pc, ARROW_DAMAGE);
}
