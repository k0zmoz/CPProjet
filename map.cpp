...
Map::Map()
{
  ...
  int array__size = 500*502; // define the size of character array_
	array_ = new char[array__size]; // allocating an array_ 
	int position = 0; //this will be used incremently to fill characters in the array_ 
  
	ifstream fin("carte23.txt");
 	if(fin.is_open())
	{
		while(!fin.eof() && position < array__size)
		{
			fin.get(array_[position]); //reading one character from file to array_
			position++;
		}
		array_[position-1] = '\0'; //placing character array_ terminating character
