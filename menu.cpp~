	...
	if (!background_img_->LoadFromFile("Menu/background.png")) // Si le chargement a échoué
	{
  	...
  	switch(state)
  	{
		...
		cout << "CASE START OK\n";
		  ...
		  buttons_.push_front(new Button(Exit2));
  	...
  	/*play_exitsel_img_ = new sf::Image;
	play_exitsel_spr_ = new sf::Sprite(*play_exitsel_img_);
	if (!play_exitsel_img_->LoadFromFile("Menu/menu_exitsel.png")) // Si le chargement a échoué
	{
		cout<<"Erreur durant le chargement de l'image"<<endl;
		exit(EXIT_FAILURE); // On ferme le programme
	}
  	play_exitsel_spr_->SetSubRect(IntRect(0, 0, 800, 600));
  	
  	
  	resume_exit_img_ = new sf::Image;
	resume_exit_spr_ = new sf::Sprite(*resume_exit_img_);
	if (!resume_exit_img_->LoadFromFile("Menu/resume_exit.png")) // Si le chargement a échoué
	{
		cout<<"Erreur durant le chargement de l'image"<<endl;
		exit(EXIT_FAILURE); // On ferme le programme
	}
  	resume_exit_spr_->SetSubRect(IntRect(0, 0, 800, 600));
  	
  	
  	resumesel_exit_img_ = new sf::Image;
	resumesel_exit_spr_ = new sf::Sprite(*resumesel_exit_img_);
	if (!resumesel_exit_img_->LoadFromFile("Menu/resume_exitsel.png")) // Si le chargement a échoué
	{
		cout<<"Erreur durant le chargement de l'image"<<endl;
		exit(EXIT_FAILURE); // On ferme le programme
	}
  	resumesel_exit_spr_->SetSubRect(IntRect(0, 0, 800, 600));
  	
  	
  	resume_exitsel_img_ = new sf::Image;
	resume_exitsel_spr_ = new sf::Sprite(*resume_exit_img_);
	if (!resume_exitsel_img_->LoadFromFile("Menu/resumesel_exit.png")) // Si le chargement a échoué
	{
		cout<<"Erreur durant le chargement de l'image"<<endl;
		exit(EXIT_FAILURE); // On ferme le programme
	}
  	resume_exitsel_spr_->SetSubRect(IntRect(0, 0, 800, 600));
  	*/
...
{ 
	switch(gs)
	{
		...
		case Echap : 		cout << "Pause OK" << "\n Pause";
					rt->Draw(*background_spr_);
					rt->Draw(*resume_exit_red_spr_);
					
					break;
					
		case End : 		//cout << "End OK" << "\n End"; 
					rt->Draw(*background_spr_);
					rt->Draw(*princess_spr_);
					break;

		default :   break; // cout << "default OK" << "\n default";
	}
